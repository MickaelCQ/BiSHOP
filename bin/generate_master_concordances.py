...
COHORTS = {
    "SLA": {
        ...
        "bed_legacy": "legacy_panels/bed/SLA_Panel_hg19_curated.bed", # <-- Le BED en hg19 !
        "pairs": [
    ...
    "TSA": {
        ...
        "bed_legacy": "legacy_panels/bed/TSA_Panel_hg19_curated.bed", # <-- Le BED en hg19 !
        "pairs": [
    ...
    "PGX": {
        ...
        "bed_legacy": "legacy_panels/bed/PGX_Panel_GRCh38_curated.bed", # <-- PGX est resté en hg38 !
        "pairs": [
...
for panel, cfg in COHORTS.items():
    print(f"\n=== Relance Mosdepth avec correction d'assemblage : {panel} ===")
    bed_wes, bed_legacy = os.path.join(BASE, cfg["bed_wes"]), os.path.join(BASE, cfg["bed_legacy"])
    
    # Le fichier WES dicte l'ordre absolu des lignes
    ...
    for wes_bam, leg_bam, sample_id in cfg["pairs"]:
        ...
        # Merge via Pandas sur le "region_id" -> Impossible de décaler les lignes !
        if w_tsv:
    ...
    print(f"✅ Matrice corrigée et blindée : {out_csv}")
